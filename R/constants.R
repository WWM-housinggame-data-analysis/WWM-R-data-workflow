...
EXPECTED_COMPLET_ROUNDS <- EXPECTED_ROUNDS[2 : length(EXPECTED_ROUNDS)]
EXPECTED_INTERM_ROUNDS <- EXPECTED_ROUNDS[2 : (length(EXPECTED_ROUNDS) - 1)]
FINAL_ROUND_CHOICE <- c(F, T)
names(FINAL_ROUND_CHOICE) <- AVAILABLE_QUESTIONS
...
PLAYER_AGGREGATION_OPTIONS <- c(INCOME_GRP_COL, PLAYER_CODE_COL)
names(PLAYER_AGGREGATION_OPTIONS) <- c("Welfare Class", "Table Player")
