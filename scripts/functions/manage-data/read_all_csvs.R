read_all_csvs <- function(folder_path = "local path", folder_pattern = "csv_folder") {
  ...
  # Load required libraries -> include in tests
  if (!requireNamespace("readr", quietly = TRUE)) stop("Package `readr` not found. Please run `install.packages('readr') to proceed")
